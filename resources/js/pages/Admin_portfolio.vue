 ...
 <v-card-title>
        ...
        <v-card-actions>
           ...
           <button><router-link class="item"
            ...
            > Update</router-link>  </button> <br>
             <b-button class="mod"  @click="$bvModal.show('bv-modal-example')"><v-icon
      large
      color="teal darken-2"
    >
      mdi-delete
    </v-icon>Delete item</b-button>
    <b-modal id="bv-modal-example" hide-footer>
    <template #modal-title>
      Delete item
    </template>
    <div class="d-block text-center">
      <h3>Do you want to delete this {{portfolio.work_name}} portfolio item?</h3>
    </div>
    <b-button class="btn btn-primary mt-3" block @click="$bvModal.hide('bv-modal-example')">Cancel</b-button>
    <b-button class="btn btn-danger mt-3" block   @click="deletePortfolio(portfolio.id)">Delete</b-button>
  </b-modal>
        </v-card-actions>
      </v-card>
    </v-row>
