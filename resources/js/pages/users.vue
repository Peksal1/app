...
<router-link class="registeruser" :to="{ name: 'registeruser' }"
      ...
      <h1>Users:</h1>
<div class="col-sm-5 col-md-6 col-12 pb-4" v-for="user in users"
        v-bind:key="user.id">
                <div class="comment mt-4 text-justify float-left"> <img src="https://t3.ftcdn.net/jpg/03/46/83/96/360_F_346839683_6nAPzbhpSkIpb8pmAwufkC7c5eD7wYws.jpg" alt="" class="rounded-circle" width="40" height="40">
                    <h4>{{ user.name }}</h4> <span>- Registered on the website on: {{ user.created_at}}</span> <br>
    ...
    <b-button class="btn btn-danger mt-3" block   @click="deleteUser(user.id)">delete</b-button>
  </b-modal>
    </div>
